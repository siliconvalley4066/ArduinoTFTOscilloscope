...
void freqDuty() {                               // 周波数とデューティ比を求める detect frequency and duty cycle value from waveform data
  ...
  float fhref;
  if (rate > 16) {      // Equivalent Time sampling
    fhref = ethref();
  } else {
    fhref = (float) pgm_read_dword(HREF + rate);
  }
  waveFreq = 30.0e6 / (fhref * pPeriod); // frequency
...
float ethref() {
  float href = 1.875; // micro second
  if (rate == 18) href *= 2.0;
  else if (rate == 19) href *= 5.0;
  else if (rate == 20) href *= 10.0;
  else if (rate == 21) href *= 20.0;
  else if (rate == 22) href *= 50.0;
  else if (rate == 23) href *= 100.0;
  return href;
}
